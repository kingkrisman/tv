...
type Channel = { id: string; name: string; logo?: string; group?: string; country?: string; language?: string; url: string; sources?: string[] };
...
export default function Index() {
  ...
  const [streamHealth, setStreamHealth] = useState<Record<string, "checking" | "online" | "degraded" | "offline">>({});
  ...
  const countries = useMemo(() => ["All countries", ...Array.from(new Set(channels.flatMap((channel) => channel.country?.split(",").map((value) => value.trim()) ?? []).filter(Boolean)))], [channels]);
  const languages = useMemo(() => ["All languages", ...Array.from(new Set(channels.flatMap((channel) => channel.language?.split(",").map((value) => value.trim()) ?? []).filter(Boolean)))], [channels]);
  const filteredChannels = useMemo(() => {
    const search = deferredQuery.toLowerCase().trim();
    return channels.filter((channel) => (!showFavorites || favoriteIds.includes(channel.id)) && (showFavorites || activeGroup === "All channels" || channel.group === activeGroup) && (countryFilter === "All countries" || channel.country?.split(",").map((value) => value.trim()).includes(countryFilter)) && (languageFilter === "All languages" || channel.language?.split(",").map((value) => value.trim()).includes(languageFilter)) && (!search || `${channel.name} ${channel.group} ${channel.country} ${channel.language}`.toLowerCase().includes(search)));
  }, [activeGroup, channels, countryFilter, deferredQuery, favoriteIds, languageFilter, showFavorites]);

  useEffect(() => {
    setVisibleCount(60);
    setRenderAll(false);
  }, [activeGroup, countryFilter, deferredQuery, languageFilter]);

  const selectChannel = (channel: Channel) => {
    setStreamError(false);
    setSourceIndex(0);
    setStreamHealth((health) => ({ ...health, [channel.id]: "checking" }));
  ...
  const activeSources = activeChannel?.sources?.length ? activeChannel.sources : activeChannel?.url ? [activeChannel.url] : [];
  const activeSource = activeSources[sourceIndex];
  const handleStreamReady = () => {
    if (activeChannel) setStreamHealth((health) => ({ ...health, [activeChannel.id]: sourceIndex > 0 ? "degraded" : "online" }));
  };
  const handleStreamError = () => {
    if (sourceIndex < activeSources.length - 1) {
      setSourceIndex((index) => index + 1);
      setPlaybackAttempt((attempt) => attempt + 1);
      setStreamHealth((health) => activeChannel ? { ...health, [activeChannel.id]: "degraded" } : health);
      return;
    }
    setStreamError(true);
    if (activeChannel) setStreamHealth((health) => ({ ...health, [activeChannel.id]: "offline" }));
  };
